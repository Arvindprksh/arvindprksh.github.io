...
\\( \min_{G} \max_{D} V(G,D) = \mathbb{E} {x~p_{data}(x)} [logD(x)] + \mathbb{E} {z~p_{z}(z)} [log(1-D(G(z))] \\) is reformulated as :

\\( C(G) = \max_{D} V(G,D) \
         = \mathbb{E} {x\~p_{data}} [log(D^\*_{G}(x))] + \mathbb{E} {z\~p_{z}} [log(1-D^\*_{G}(G(z)))]  \
         = \mathbb{E} {x\~p_{data}} [log(D^\*_{G}(x))] + \mathbb{E} {x\~p_{g}} [log(1-D^\*_{G}(x))]  \
         = \mathbb{E} {x\~p_{data}} [log(p_{data}(x) / p_{data}(x) + p_{g}(x))] + \mathbb{E} {x\~p_{g}} [log(p_{g}(x) / p_{data}(x) + p_{g}(x))]  
\\)
...
**proof.** for \\( p_{g} = p_{data}, p_{data}(x) / (p_{data}(x) + p_{g}(x)) =  p_{g}(x) / (p_{data}(x) + p_{g}(x)) = 1/2 \\), C(G) is equal to : 
 ...
 \\( \mathbb{E} {x~p_{data}} [-log2] + \mathbb{E} {x~p_{g}} [-log2] = -log4 \\)
