...
First of all, This GAN(Genrative Adversarial Nets) is a framework which explains adversarial training for recover traing data distribution so In this paepr, The generator G implicitly defines ** a probability distribution \\( p_{g} \\)** as the distribution of the samples \\( G(z) \\) when \\( z ~ p_{z} \\).
...
So The G and D complementaray help improve the performance of each other. 
...
In this paper, both of G and D is multiplayer perceptrons and the output of function V(G,D) is a scalar where measure wheter the sample is from traning data or from sample generator create.
...
> So In the paper, they used a trick. it maximzes log(D(G(z))) rather than minimize log(1-D(G(z))) like the curve above in the figure above.
...
(a), (b), (c), (d) in turn update the adversarial traing. (a) currently discriminative doesn't be trained. (b) the discriminator is trained so the blue dotted line is chaneg.
...
##### Proposition 1. For G fixed, the optimal discriminator D is \\( D^*_{G}(x) = p_{data}(x) / p_{data}(x) + p_{g}(x) \\)
...
\\( For any (a,b) \in \R^2, the function y -> a*log(y) + b*log(1-y) achieves maximum value in \[0,1\] at a/(a+b) )\\.

if you get derivative of the function y -> a*log(y) +  b*log(1-y) with respect to y for \\( argmax_{y} \\), suppose the base of logarithms above is e.
...
the funtion y -> a*log(y) + b*log*(1-y) reache maximum in \[0,1\] at a/(a+b).


\\( \min_{G}\max_{D}V(G,D) = \mathbb{E}_{x~p_{data}(x)}[logD(x)] + \mathbb{E}_{z~p_{z}(z)[log(1-D(G(z))] \\) is reformulated as :

\\( C(G) = \max_{D}V(G,D)
         = \mathbb{E}_{x\~p_{data}}[log(D^\*_{G}(x))] + \mathbb{E}_{z\~p_{z}}[log(1-D^\*_{G}(G(z)))]  
         = \mathbb{E}_{x\~p_{data}}[log(D^\*_{G}(x))] + \mathbb{E}_{x\~p_{g}}[log(1-D^\*_{G}(x))]
         = \mathbb{E}_{x\~p_{data}}[log(p_{data}(x) / p_{data}(x) + p_{g}(x))] + \mathbb{E}_{x\~p_{g}}[log(p_{g}(x) / p_{data}(x) + p_{g}(x))]  
\\)
...
**proof.** for \\( p_{g} = p_{data}, p_{data}(x) / p_{data}(x) + p_{g}(x) =  p_{g}(x) / p_{data}(x) + p_{g}(x) = 1/2 \\), C(G) is equal to : 
 ...
 \\( \mathbb{E}_{x~p_{data}}[-log2] + \mathbb{E}_{x~p_{g}}[-log2] = -log4 \\)
 
 and then if you subtract -log4 from \\( C(G) = V(D^*_{G}, G), we obatain : \\)
 
\\( C(G) = -log4 + log4 + \mathbb{E}_{x~p_{data}}[log(p_{data}(x) / p_{data}(x) + p_{g}(x))] + \mathbb{E}_{x~p_{data}}[log(p_{data}(x) / p_{data}(x) + p_{g}(x))]
         = -log4 + \mathbb{E}_{x~p_{data}}[log(p_{data}(x) / p_{data}(x) + p_{g}(x))] + log2 + \mathbb{E}_{x~p_{data}}[log(p_{data}(x) / p_{data}(x) + p_{g}(x))] + log2
         = -log4 + KL(p_{data} || (p_{data}+p_{g})/2) + KL(p_{g} || (p_{data}+p_{g})/2) 
    and then, 
    JSD(P || Q) = 1/2*D_{KL}(P||M) + 1/2*D_{KL}(Q||M) where M = 1/2*(P+Q)
