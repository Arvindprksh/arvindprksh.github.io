...
\\( \min_{G} \max_{D} V(G,D) = \EX_{x \sim p_{data}(x)} [logD(x)] + \EX_{z \sim p_{z}(z)}[log(1-D(G(z))] \\)
...
if you get derivative of the function y -> a*log(y) +  b*log(1-y) with respect to y for \\( \argmax_y f(Y) \\), suppose the base of logarithms above is e.
