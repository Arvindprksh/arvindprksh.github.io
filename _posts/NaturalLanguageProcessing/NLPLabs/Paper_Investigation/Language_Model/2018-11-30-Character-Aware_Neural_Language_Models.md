...
\\( F(x) = \frac{exp(h_{t} \cdot s^r + t^r)}{\sum_{r\textprime=1}^n exp(h_{t} \cdot s^r\textprime + t^r\textprime)} \\)

\\( G(x) = \frac{exp(h_{t} \cdot P_{r}^j + q_{r}^j)}{\sum_{j'\inV_{r}} exp(h_{t} \cdot P_{r}^j' + q_{r}^j')} \\)  
