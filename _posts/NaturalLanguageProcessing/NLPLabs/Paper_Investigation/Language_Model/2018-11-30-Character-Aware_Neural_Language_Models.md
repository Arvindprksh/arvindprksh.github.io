...
The \\( Pr(W_{i} = j \| W_{1:t}) = \frac{exp(h_{t} \cdot s^r + t^r)}{\sideset{}{_r'=1^c}\sum exp(h_{t} \cdot s^r' + t^r')} \times \frac{exp(h_{t} \cdot P_{r}^j + q_{r}^j)}{\sideset{}{_j'\inV_{r}}\sum exp(h_{t} \cdot P_{r}^j' + q_{r}^j')} \\)
