...
The one is to be learned from count-based method like latent semantic anlaysis-LSA(Deer-wester et al., 1990) and hyperspace analogue to Language-HAL(Lund and Burgess, 1996).
  ...
  - based on frequencies of terms.
    ...
    - PPMI(Bullinaria and Levy 2007), square root type HPCA-Hellinger PCA(Lebret and Collobert, 2014) and so on.
...
W\textsubscript{j} is context word vector for the context of ith row vector on co-occurence matrix.

The cost function calculate the least square of the dot product of W\textsubscript{i} and W\textsubscript{j} - log(X\textsubscript{ij})
